...
class GraphConv(torch.nn.Module):
  ...
  def forward(self, x: torch.Tensor, octree: OctreeD, depth: int):
    ...
    has_node_type = graph.depth_max - graph.depth_min > 0
    ...
    # add self-loops
    index = graph.arange(graph.nnum, dtype=torch.int64, device=x.device)
...
class GraphDownsample(torch.nn.Module):
  ...
  def forward(self, x: torch.Tensor, octree: OctreeD, depth: int):
    ...
    outd = x.new_zeros(octree.nnum[depth-1], x.shape[1])
    leaf_mask = octree.child[depth-1] < 0
